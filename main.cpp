...
//       yuki_new_features__thread_test::asyncTest(std::launch::deferred);
    ...
    *****/
    ...
    {
//        std::vector<std::future<int>> vv;
        std::vector<std::promise<int>> v;
        std::vector<std::future<int> > v_f;
        yuki_new_features__thread_test::ThreadPoolTestPromise t(4);
        t.initThread(4);
        for(size_t i = 0; i < 4; ++i){
            std::promise<int> temp;
            v_f.push_back(temp.get_future());
            t.pushTask(temp, i);
